...
class ImplementationChecker {
    fun checkedIpv4(ip:String):Boolean{
        if (ip.isEmpty()){
            return false
        }
        ...
        for (seg in ipSeg) {
            ...
            if (segInteger <0 || segInteger >255)
            {
                return false
            }
        }
    ...
    fun checkedSudoku(game: List<List<Char>>): Boolean {
        val size = game.size
        // check all len of rows have the same len of game size
        for (row in game) {
        ...
        if (subBoxesSizeDouble * subBoxesSizeDouble != size.toDouble())
            return false
            ...
            for (c in 0 until size step subBoxesSize) {
                ...
                for (rSub in 0 until subBoxesSize) {
                    for (cSub in 0 until subBoxesSize) {
                       ...
                       // print(charSub)
                        if (charSub != '-' && subSet.contains(charSub))
                            return false
